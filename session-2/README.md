...
## 1. What are we going to build
   ...
   In session 1, we saw how to approximate linear and quadratic functions using Neural networks. This was a simple enough project to introduce me to Tensorflow and how to use it to build Neural Network models. Now it is time to move to a more fun project that will be very hard (if possible to do with classic programming). Let's build a system when given an image of a chart that can tell us what type of chart is in the image. For example, If the system receives this image: 
...
<img src="images/pie.jpg" height="250" width="250">
   ...
   First, we need to consider the input to our model. In all the cases we have seen so far, the input was quite simple: either 1 or 2 simple numbers, but in this case, what would it be?
	We can not just feed the image as an image to the model for training; we need to represent it in numbers. The easiest way is to feed in the pixel values as the input to the model. If we have a true-colour image, we can represent every pixel using three integer values to represent the RGB value of the pixel. In other words, if our image is W width and H height in pixels, we represent it as W*H*3 numbers. We can feed it to the Neural network and start the training process.
...
**True Positives (TP):** 
   ...
   These are the correctly predicted positive values which mean that the value of the actual class is yes and the value of the predicted class is also yes. E.g. if the actual class value is Bar charts, and the predicted class is Bar charts.
...
The First line uses the model to predict all the values for our testing data by calling ``predict_generator`` on our model and passing it to the generator.
...
An interesting observation here is that our training precision is 100%, while our testing precision is 78%. Although 78% is good, a big difference between model training performance and testing performance indicates a problem called Overfitting.
...
3- Remove useless features, for example, does colour matter in your classification, maybe you should switch to grayscale instead of RGB
...
Another important aspect to keep in mind when dealing with multiple class classification is that if we find that our model performs badly for one class, it is important to check what it confuses this class with. In other words imagine we were detecting Bar, Pie, and Line charts then we find that our model performs bad with line chart classification, it will be very helpful to know what does it confuse line charts with. For example, does it tend to misclassify line charts as bar charts, does it tend to think that pie charts are line charts and so on? This helps us to decide what actions do we need to take to deal with the issue.

This is where the concept of a confusion matrix becomes very handy. The easiest way to understand the confusion matrix is to visualize it:

<p align="center"> 
<img src="images/confusionmatrix.png" height="300" width="500">
</p>   

A quick glance at the confusion matrix will allow you to compare the predicted classes to the actual ones, what classes get misclassified, and what was the misclassification. We want the Diagonal to have the highest number since this reflects that prediction matched the true class.

The module we used before to print our precision and F1 score comes with a handy function for showing the confusion matrix. Here is how we can show our confusion matrix
